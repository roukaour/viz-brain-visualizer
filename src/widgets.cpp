...
void Toggle_Switch::draw() {
	...
	Fl_Boxtype pb = Fl::scheme() ? value() ? OS::current_theme() == OS::GREYBIRD ? OS_DEFAULT_BUTTON_BOX :
...
Sidebar::Sidebar(int x, int y, int w, int h, const char *l) : Fl_Group(x, y, w, h, l), _spacer(x, y + h, w, 0) {
	labeltype(FL_NO_LABEL);
	box(OS_PANEL_THIN_UP_FRAME);
	resizable(_spacer);
}
